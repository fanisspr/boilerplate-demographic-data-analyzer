...
def calculate_demographic_data(print_data=True):
    ...
    percentage_bachelors = df.loc[df['education'] ==
                                  'Bachelors', 'education'].count() / df.shape[0]
    ...
    higher_education_perc = num_higher_education / df.shape[0]

    lower_education = df.loc[~df['education'].isin(
        ['Bachelors', 'Masters', 'Doctorate'])]
    num_lower_education = lower_education.shape[0]
    lower_education_perc = num_lower_education / df.shape[0]

    # percentage with salary >50K
    higher_education_rich = higher_education.loc[higher_education['salary']
                                                 == '>50K'].shape[0] / num_higher_education
    higher_education_rich = round(higher_education_rich*100, 1)

    lower_education_rich = lower_education.loc[df['salary']
                                               == '>50K'].shape[0] / num_lower_education
    ...
    rich_percentage = min_workers.loc[df['salary']
                                      == '>50K'].shape[0] / num_min_workers
    rich_percentage = round(rich_percentage*100, 1)

    # What country has the highest percentage of people that earn >50K?
    highest_earning_countries = df.loc[df['salary'] == '>50K',
    ...
    highest_earning_country_percentage = highest_earning_countries[0].round(
        3) * 100

    # Identify the most popular occupation for those who earn >50K in India.
    top_IN_occupations = df.loc[(df['salary'] == '>50K') & (
